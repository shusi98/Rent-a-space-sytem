...
else if( isset($_GET['act']) && !empty($_GET['act']) && isset($_GET['pid']) && !empty($_GET['pid']) )
{
	...
	if($action == "del")
	{
		$multiQuery = "delete from property where PropertyID = '$propertyId';delete from rental where PropertyID = '$propertyId';delete from payment where PropertyID = '$propertyId';delete from image where PropertyID = '$propertyId';";
...
else if( isset($_GET['act']) && !empty($_GET['act']) && isset($_GET['tid']) && !empty($_GET['tid']) )
{
	...
	if($action == "del")
	{
		...
		{
			/*for($x = 0; $x < count($_SESSION["RentingRentalID"]); $x++)
			{
				$propertyId = $_SESSION["RentingRentalID"][$x];
				$multiQuery .= "update payment set AmountDue = '0', DateDue = 'NULL' where PropertyID = '$propertyId';delete from rental where PropertyID = '$propertyId';";
			}*/
			foreach($_SESSION["RentingRentalID"] as $value)
			{
				$multiQuery .= "update payment set AmountDue = '0', DateDue = 'NULL' where PropertyID = '$value';delete from rental where PropertyID = '$value';";
			}
			$multiQuery .= "delete from user where UserID = '$tenantId';";
			unset($_SESSION["UserCurrentlyRenting"]);
			unset($_SESSION["RentingRentalID"]);
		}
		else
		{
			$multiQuery = "delete from user where UserID = '$tenantId';";
...
<div class="container">
	...
	<div class="d-flex" style="margin-top:15px;">
		<h3><?php echo $pageDescription;?></h3>
	</div>
	<div class="d-flex" style="margin-top:15px;">
		<ul class="breadcrumb">
